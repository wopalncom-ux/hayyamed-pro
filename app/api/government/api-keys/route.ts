...
const VALID_SCOPES = ["read:registry", "read:compliance"] as const;

async function getGovernmentOrg(userId: string) {
...
export async function GET() {
  ...
  const orgId = await getGovernmentOrg(user.id);
  if (!orgId) return NextResponse.json({ error: "No government organisation" }, { status: 403 });
...
export async function POST(req: NextRequest) {
  ...
  await logAudit({
    actorAuthId: user.id,
    action: "government.api_key.created",
...
export async function DELETE(req: NextRequest) {
  ...
  await logAudit({
    actorAuthId: user.id,
    action: "government.api_key.revoked",
    targetTable: "api_keys",
    targetId: id,
