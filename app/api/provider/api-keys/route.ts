...
const VALID_SCOPES = ["read:courses", "read:enrollments"] as const;

async function getProviderOrg(userId: string) {
...
export async function GET() {
  ...
  if (!providerId) return NextResponse.json({ error: "No active provider account" }, { status: 403 });
...
export async function POST(req: NextRequest) {
  ...
  await logAudit({
    actorAuthId: user.id,
    action: "provider.api_key.created",
...
export async function DELETE(req: NextRequest) {
  ...
  await logAudit({
    actorAuthId: user.id,
    action: "provider.api_key.revoked",
    targetTable: "api_keys",
    targetId: id,
