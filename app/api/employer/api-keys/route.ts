...
const VALID_SCOPES = ["read:staff", "read:compliance", "read:licenses"] as const;
...
export async function POST(req: NextRequest) {
  ...
  await logAudit({
    actorAuthId: user.id,
    action: "employer.api_key.created",
    targetTable: "api_keys",
    targetId: created.id,
    metadata: { name: parsed.data.name, scopes: parsed.data.scopes },
  });

  // Return full key ONCE — hash is all that's stored
...
export async function DELETE(req: NextRequest) {
  ...
  await logAudit({
    actorAuthId: user.id,
    action: "employer.api_key.revoked",
    targetTable: "api_keys",
    targetId: id,
